//! Initialize ant-army for a project
...
pub async fn run(database_url: Option<String>, force: bool) -> Result<()> {
    println!("🐜 Initializing Ant Army for this project...\n");
    ...
    let db_url = database_url.unwrap_or_else(|| {
        format!(
            "postgresql://ant_army:ant_army_dev@localhost:5432/postgres"
        )
    });
    ...
    if db_exists {
        ...
        let template_exists: bool = sqlx::query_scalar(
            "SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = 'ant_army_template')"
        )
        .fetch_one(&pool)
        .await?;
        ...
        if !template_exists {
            anyhow::bail!(
                "Template database 'ant_army_template' not found. \
        ...
        let create_query = format!(
            "CREATE DATABASE {} TEMPLATE ant_army_template",
            config.db_name
        );
    ...
    let schema_exists: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'ants')"
    )
    ...
    // Print summary
    println!("\n🎉 Ant Army initialization complete!");
    println!("\nProject Details:");
    ...
    println!("\nNext Steps:");
    println!("  1. Create a task: ant-army task create \"Your task description\"");
    println!("  2. Check status:  ant-army task list");
    println!("  3. View workers:  ant-army workers list");
